...
download_if_not_exists() {
    ...
    if [ -f "$filename" ]; then
        ...
        wget -q --show-progress "$url" || error_exit "[$filename] 다운로드에 실패했습니다. 네트워크 상태나 링크를 확인해 주십시오."
    fi
}
...
# 2) 필수 패키지 및 최신 컴파일러(GCC 8) 설치
echo "[작업 1] 패키지 및 컴파일러 업데이트를 진행합니다."
...
yum install -y --nogpgcheck --disablerepo=*wandisco*,*icinga* devtoolset-8-gcc devtoolset-8-gcc-c++ devtoolset-8-binutils bison python3 wget bzip2 patchelf gawk rpm2cpio cpio || error_exit "컴파일에 필요한 필수 패키지 설치에 실패했습니다."
